...
- Return ONLY the raw <svg>...</svg> code. No markdown, no triple backticks.`;
    ...
    async function fetchQuizBatch() {
      ...
      const subjectHints = {
        'English': 'English language: grammar rules, vocabulary, spelling, synonyms/antonyms, sentence structure, punctuation, idioms',
        'Chinese': 'Chinese language (Mandarin): vocabulary, pinyin, stroke order, simple characters, sentence structure, opposites, colours/numbers in Chinese',
        'Pure Chinese': 'PURE MANDARIN CHINESE — CRITICAL: Write EVERY question AND every answer choice ENTIRELY in Simplified Chinese characters. Absolutely NO English, NO pinyin ever. Topics: vocabulary, colours, numbers, animals, food, opposites, family.',
        'Math': 'Mathematics: arithmetic, shapes, patterns, algebra, measurement',
        'Science': 'Science: nature, animals, plants, physics, chemistry, body, space',
        'General Knowledge': 'General Knowledge: world geography, history, culture, sports, famous people',
      };

      const slotDescriptions = slots
        .map((s, i) => `  Question ${i + 1}: subject = "${s}" (${subjectHints[s] || `CUSTOM TOPIC: "${s}" — this question MUST be exclusively about ${s}. Every aspect of the question and all answer choices must relate directly to ${s}. Do NOT generate questions about unrelated subjects.`})`)
        .join('\n');
      ...
      displayAnswers.forEach((answer, idx) => {
        ...
        if (hasImageType && imageKeyword) {
          ...
          Object.assign(imgWrapper.style, {
            ...
            overflow: 'hidden',
            maxHeight: '35vh',
            height: Math.floor(window.innerHeight * 0.35) + 'px',
            background: 'linear-gradient(90deg, #1e2d42 25%, #243553 50%, #1e2d42 75%)'
          });
